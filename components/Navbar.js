...
function initNavbar() {
  ...
  var currentIndex = 0;
  var totalItems = navItems.length + 2; // search input + profile icon
  ...
  profileIcon.addEventListener("focus", function() {
    profileIcon.classList.add("active");
    isNavbarFocused = true;
  });
  profileIcon.addEventListener("blur", function() {
  ...
  function updateNavbarActive(page) {
    ...
    currentIndex = index + 1; // +1 because search input is index 0
    highlightNavItem(currentIndex);
    isNavbarFocused = true;
  }
  ...
  function handleSidebarKeys(e) {
    ...
    var activeIndex = items.findIndex(item => item.classList.contains("active"));
