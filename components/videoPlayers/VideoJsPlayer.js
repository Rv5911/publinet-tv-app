function VideoJsPlayer(poster = "") {
  ...
  const isYouTube =
    srcUrl.includes("youtube.com") || srcUrl.includes("youtu.be");

  console.log("srcUrl", srcUrl);
  // 🔴 YouTube Trailer Handling - MUST BE AT THE TOP
  // This prevents the code below (playlist parsing) from crashing if data is missing
  if (fromValue === "trailer_movie" || fromValue === "trailer_series") {
    if (isYouTube) {
      if (typeof tizen !== "undefined" && tizen.application) {
        // Tizen-specific implementation for opening external URLs
      ...
      console.warn("Non-YouTube trailer URL or missing URL:", srcUrl);
      // Optional: try opening non-YouTube links externally too
      if (typeof tizen !== "undefined" && tizen.application) {
        var appControl = new tizen.ApplicationControl(
          "http://tizen.org/appcontrol/operation/view",
          srcUrl,
    ...
    // Navigate back to the details page immediately
    ...
    return ""; // Stop execution here and return empty so no player HTML is rendered
  }
  ...
  // Wrap potentially dangerous parsing in try-catch/checks
  let playingItemData = {};
  try {
    const item = localStorage.getItem("playingItemData");
    if (item) playingItemData = JSON.parse(item);
  } catch (e) {
    console.warn("Error parsing playingItemData", e);
  }
  const titleText = playingItemData.title || playingItemData.name || "";

  let currentPlaylistName = "";
  try {
    const playlistItem = localStorage.getItem("selectedPlaylist");
    if (playlistItem) {
      currentPlaylistName = JSON.parse(playlistItem).playlistName;
    }
  } catch (e) {
    console.warn("Error parsing selectedPlaylist", e);
  }

  let currentPlaylist = null;
  try {
    const playlistsStr = localStorage.getItem("playlistsData");
    if (playlistsStr) {
      currentPlaylist = JSON.parse(playlistsStr).filter(
        (pl) => pl.playlistName === currentPlaylistName
      )[0];
    }
  } catch (e) {
    console.warn("Error parsing playlistsData", e);
  }
  ...
  function initPlayer(attempt = 0) {
    ...
    player.on("error", () => {
      ...
      if (errorDialog) {
        ...
        if (errorText) {
          errorText.innerHTML = `⚠️ Error: ${
            error.message || "Something went wrong"
          } <br/><span style="font-size: 0.8em; opacity: 0.8;">Code: ${
            error.code || "Unknown"
          }</span>`;
        }
      }
