            ...
            try {
                ...
                if (data && data.user_info) {
                    if (data.user_info.auth === 1 && data.user_info.status === "Active") {
                        ...
                        setTimeout(() => {
                            const previousMovieStreamId = localStorage.getItem(
                                "previousMovieStreamId"
                            );
                            const previousSeriesStreamId = localStorage.getItem(
                                "previousSeriesStreamId"
                            );
                            localStorage.setItem("isLogin", true);

                            if (previousMovieStreamId && window.allMoviesStreams) {
                                const movie = window.allMoviesStreams.find(
                                    (m) => String(m.stream_id) === String(previousMovieStreamId)
                                );
                                if (movie) {
                                    localStorage.setItem(
                                        "selectedMovieId",
                            ...
                            if (previousSeriesStreamId && window.allSeriesStreams) {
                                const series = window.allSeriesStreams.find(
                                    (s) => String(s.series_id) === String(previousSeriesStreamId)
                                );
                                if (series) {
                                    localStorage.setItem(
                                        "selectedSeriesId",
