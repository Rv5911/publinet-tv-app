...
const handleChannelChange = (direction) => {
  ...
  if (isAdultChannel && parentalEnabled) {
    if (selectedCategoryId === "All") {
      isChannelUnlocked = unlockedLiveAdultChannelsInAll.has(String(streamId));
      unlockSet = unlockedLiveAdultChannelsInAll;
    } else if (selectedCategoryId === "favorites") {
      isChannelUnlocked = unlockedLiveAdultChannelsInFavorites.has(String(streamId));
      unlockSet = unlockedLiveAdultChannelsInFavorites;
    } else if (selectedCategoryId === "channelHistory") {
      isChannelUnlocked = unlockedLiveAdultChannelsInHistory.has(String(streamId));
      unlockSet = unlockedLiveAdultChannelsInHistory;
    } else {
      const category = categories.find(cat => cat.category_id === selectedCategoryId);
      isChannelUnlocked = category ? unlockedLiveAdultCatIds.has(String(category.category_id)) : true;
      unlockSet = null;
    }
  }
  ...
  if (isAdultChannel && parentalEnabled && !isChannelUnlocked && unlockSet) {
    ParentalPinDialog(
      () => {
        ...
        const logoWrapper = newCard.querySelector(".channel-card-logo-wrapper");
        if (logoWrapper) {
    ...
    if (currentStreamId !== streamId) {
      ...
      if (typeof LiveVideoJsComponent.cleanup === "function") {
        try {
          ...
          console.warn("LiveVideoJsComponent cleanup error during channel change:", err);
        }
      }
      ...
      const currentPlaylist = playlistsData.filter(pl => pl.playlistName === selectedPlaylist.playlistName)[0];
...
const handleFullscreenBorder = () => {
  ...
  const isFlowplayerFullscreen = window.livePlayer && 
...
const showPlayPauseControls = () => {
  ...
  // SHOW ASPECT RATIO BUTTONS - ADD THIS
...
const hideAllControls = () => {
  ...
  document.querySelectorAll(".play-pause-btn-focused, .videojs-aspect-ratio-btn-focused, .flow-aspect-ratio-btn-focused").forEach(btn => {
    btn.classList.remove("play-pause-btn-focused", "videojs-aspect-ratio-btn-focused", "flow-aspect-ratio-btn-focused");
  });
};
...
const getFilteredCategories = () => {
  ...
  const freshPlaylistData = JSON.parse(localStorage.getItem("playlistsData")).find(pl => pl.playlistName === currentPlaylistName);
  const updatedFavorites = freshPlaylistData ? freshPlaylistData.favoritesLiveTV : [];
...
const handleLongPressEnter = (card) => {
  ...
  if (selectedCategoryId === "channelHistory") {
    ...
    // INVALIDATE CACHE - Add this line
    invalidateCategoryCache();
    ...
    // ONLY refresh if we're in the favorites category
...
const loadMoreCategories = () => {
  ...
  const categoriesToLoad = filtered.slice(alreadyLoaded, categoryChunk * categoryPageSize);
...
const renderChannelsContent = (isAppend = false, categoryChangeOnly = false) => {
  const container = qs(".livetv-content-container");
  if (!container) return;
...
<p class="livetv-channel-category-count">${c.channels ? c.channels.length : 0}</p>
  ...
  if (inChannelList) {
    ...
    if (updatedCategories.length > 0) {
      setFocus(updatedCategories, focusedCategoryIndex, "livetv-channel-category-focused");
    }
  }
}
...
newChannels.forEach((ch) => {
  ...
  if (isAdultChannel && parentalEnabled) {
    if (selectedCategoryId === "All") {
      isChannelUnlocked = unlockedLiveAdultChannelsInAll.has(String(ch.stream_id));
    } else if (selectedCategoryId === "favorites") {
      isChannelUnlocked = unlockedLiveAdultChannelsInFavorites.has(String(ch.stream_id));
    } else if (selectedCategoryId === "channelHistory") {
      isChannelUnlocked = unlockedLiveAdultChannelsInHistory.has(String(ch.stream_id));
    } else {
  ...
  card.innerHTML = `
    <div class="channel-card-logo-wrapper ${isAdultChannel && parentalEnabled && !isChannelUnlocked ? 'channel-card-locked' : ''}">
      ...
      ${isAdultChannel && parentalEnabled && !isChannelUnlocked ? '<i class="fas fa-lock channel-card-lock-icon"></i>' : ''}
    </div>
...
if (!isAppend && !categoryChangeOnly && newChannels.length > 0) {
  const videoWrapper = qs(".livetv-video-wrapper");
  const hasActiveVideo = videoWrapper && (
...
if (removeBtn) {
  ...
  const currentPlaylistName = JSON.parse(localStorage.getItem("selectedPlaylist")).playlistName;
  const playlistsData = JSON.parse(localStorage.getItem("playlistsData"));
  const currentPlaylistIndex = playlistsData.findIndex(pl => pl.playlistName === currentPlaylistName);

  if (currentPlaylistIndex !== -1) {
    // Remove from channel history immediately
    playlistsData[currentPlaylistIndex].ChannelListLive = 
      (playlistsData[currentPlaylistIndex].ChannelListLive || []).filter(
        ch => String(ch.stream_id) !== String(streamId)
      );
    ...
    if (remainingCards.length > 0) {
      focusedCardIndex = Math.min(focusedCardIndex, remainingCards.length - 1);
      focusCards(focusedCardIndex);
    } else {
      focusCategories(focusedCategoryIndex);
    }
  ...
  if (selectedCategoryId === "favorites") {
    ...
    if (currentPlaylistIndex !== -1) {
      ...
      playlistsData[currentPlaylistIndex].favoritesLiveTV = 
        (playlistsData[currentPlaylistIndex].favoritesLiveTV || []).filter(
          fav => String(fav.stream_id) !== String(item.stream_id)
        );
      localStorage.setItem("playlistsData", JSON.stringify(playlistsData));
      ...
      const remainingCards = qsa(".channel-card");
      if (remainingCards.length > 0) {
        focusedCardIndex = Math.min(currentFocusedCardIndex, remainingCards.length - 1);
        focusCards(focusedCardIndex);
      } else {
        focusCategories(focusedCategoryIndex);
      }
  ...
  if (isAdultChannel && parentalEnabled && !isChannelUnlocked && unlockSet) {
    ParentalPinDialog(
      () => {
        ...
        const logoWrapper = card.querySelector(".channel-card-logo-wrapper");
        if (logoWrapper) {
          logoWrapper.classList.remove("channel-card-locked");
        }
...
const isFullscreen = document.fullscreenElement || 
                    ...
                    document.mozFullScreenElement ||
                    document.msFullscreenElement ||
                    (window.livePlayer && typeof window.livePlayer.fullscreen === 'function' && window.livePlayer.fullscreen());
...
if (isFullscreen) {
  ...
  if (backKeys.includes(e.key) || backKeys.includes(e.keyCode)) {
    ...
    if (isDocumentFullscreen || isFlowplayerFullscreen) {
      ...
      } else if (window.livePlayer && typeof window.livePlayer.fullscreen === 'function') {
  ...
  // Auto-hide function that hides ALL controls
  const autoHideAllControls = () => {
    if (window.livePlayer) {
      ...
      // For Flowplayer
      else if (window.livePlayer.playing && typeof window.livePlayer.playing === 'function') {
        isPlaying = window.livePlayer.playing();
      }
      
      // Hide ALL controls if video is playing
      if (isPlaying) {
        hideAllControls();
      }
    }
  ...
  if (isEnter) {
    ...
    const focusedVideoJsAspect = document.querySelector("#videojs-aspect-ratio.videojs-aspect-ratio-btn-focused");
    const focusedFlowAspect = document.querySelector("#flow-aspect-ratio.flow-aspect-ratio-btn-focused");
    ...
    if (isAspectRatioFocused) {
      ...
      setTimeout(autoHideAllControls, 3000);
      e.preventDefault();
      return;
    }
    
    // Check if play/pause button is focused
    const focusedPlayPause = document.querySelector(".play-pause-btn-focused");
    const isPlayPauseFocused = document.getElementById("live-play-pause-btn") ? 
      document.getElementById("live-play-pause-btn").classList.contains("play-pause-btn-focused") : 
      false;
    ...
    if (focusedPlayPause || isPlayPauseFocused) {
      ...
      // If no focus, show ALL controls and focus on play/pause button
      showPlayPauseControls();
  ...
  if (isUp) {
    ...
    if (focusedVideoJsAspect) focusedVideoJsAspect.classList.remove("videojs-aspect-ratio-btn-focused");
    if (focusedFlowAspect) focusedFlowAspect.classList.remove("flow-aspect-ratio-btn-focused");
  ...
  const allowedKeys = ["ArrowUp", "ArrowDown", "Enter", "VolumeUp", "VolumeDown", "AudioVolumeUp", "AudioVolumeDown"];
  if (!allowedKeys.includes(e.key) && !backKeys.includes(e.key) && !backKeys.includes(e.keyCode)) {
    e.preventDefault();
  }
  return;
}
  ...
  const channelList = qsa(".livetv-channel-category"),
        ...
        searchInput = qs("#livetv-header-search") ? qs("#livetv-header-search") : null;
  ...
  if (backKeys.includes(e.key) || backKeys.includes(e.keyCode)) {
    ...
    const isPlayerFullscreen = window.livePlayer && 
...
if (isUp) {
  ...
  const focusedFlowAspect = qs("#flow-aspect-ratio.flow-aspect-ratio-btn-focused");
  const isAspectRatioFocused = focusedVideoJsAspect || focusedFlowAspect;
  ...
  if (isAspectRatioFocused) {
    ...
    if (focusedVideoJsAspect) {
      focusedVideoJsAspect.classList.remove("videojs-aspect-ratio-btn-focused");
    }
  ...
  if (isEpgDivFocused) {
    ...
    if (epgItems.length > 0 && focusedEpgItemIndex >= 0) {
      epgItems.forEach(item => item.classList.remove("livetv-player-epg-item-focused"));
      ...
      if (focusedEpgItemIndex === 0) {
        ...
        focusedEpgItemIndex--;
        setFocus(epgItems, focusedEpgItemIndex, "livetv-player-epg-item-focused");
      }
    } else {
  ...
  if (focusedPlayPause || isPlayPauseFocused) {
    // Remove focus from play/pause button
    const playPauseBtn = document.querySelector(".play-pause-btn-focused") || 
  ...
  if (focusedFav || focusedRemove) {
    ...
    if (focusedFav) focusedFav.classList.remove("channel-fav-btn-focused");
    if (focusedRemove) focusedRemove.classList.remove("channel-remove-btn-focused");
  ...
  if (inChannelList && focusedCategoryIndex > 0) {
    ...
    document.querySelector(".live-video-player-div").style.border = "none";
  } else {
    focusSearch();
  }
  e.preventDefault();
...
if (isDown) {
  ...
  if (inVideoPlayer) {
    ...
    const retryButtonEl = document.querySelector(".retry-btn");
    if (retryButtonEl && retryButtonEl.style.display !== 'none' && retryButtonEl.offsetParent !== null) {
  ...
  if (inSearch || inMenu) {
    ...
    if (focusedCategoryIndex < currentChannelList.length - 1) {
      ...
      selectedCategoryId = currentChannelList[focusedCategoryIndex].dataset.categoryId;
    } else {
      ...
      if (currentlyLoaded < totalAvailable) {
        ...
        setTimeout(() => {
          ...
          if (updatedChannelList.length > focusedCategoryIndex + 1) {
            ...
            selectedCategoryId = updatedChannelList[focusedCategoryIndex].dataset.categoryId;
          }
        }, 50);
      } else {
...
if (isRight) {
  ...
  const videoWrapper = qs(".livetv-video-wrapper");
  const isVideoPlayerEmpty = videoWrapper && 
    (videoWrapper.textContent.includes("Please select any channel to play"));
  ...
  if (focusedFav && selectedCategoryId !== "channelHistory" && !isVideoPlayerEmpty) {
    focusedFav.classList.remove("channel-fav-btn-focused");
    const fullscreenBtn = qs("#live-fullscreen-btn");
    if (fullscreenBtn) {
      setFlags(false, false, false, false, true);
      qsa(".live-control-btn").forEach(btn => btn.classList.remove("live-control-btn-focused"));
      qsa(".channel-card-focused").forEach(btn => btn.classList.remove("channel-card-focused"));
      fullscreenBtn.classList.add("live-control-btn-focused");
      document.querySelector(".live-video-player-div").style.border = "6px solid var(--gold)";
  ...
  if (inChannelList) {
    const catEl = document.querySelector(".livetv-channel-category-active");
    const countElement = catEl.querySelector(".livetv-channel-category-count");
        ...
        if (isAdult && parentalEnabled && !isUnlocked) {
          ParentalPinDialog(
            () => {
              ...
              const lockEl = catEl.querySelector(".livetv-category-lock-icon");
              if (lockEl) lockEl.remove();
              renderChannels(false, true);
...
if (inVideoPlayer) {
  ...
  const playerDiv = document.querySelector(".live-video-player-div");
  const hasGoldBorder = playerDiv && playerDiv.style.border === "6px solid var(--gold)";
  ...
  const hasVideoPlayer = document.querySelector('video') || 
                        (window.livePlayer && typeof window.livePlayer.fullscreen === 'function');
  ...
  if ((focusedBtn && fullscreenBtn && focusedBtn === fullscreenBtn) || 
...
if (isEnter) {
  ...
  if (inVideoPlayer && !isFlowplayerFullscreen) {
    ...
    if (focusedBtn && (focusedBtn.id === "live-prev-btn" || focusedBtn.id === "live-next-btn")) {
      focusedBtn.click();
      e.preventDefault();
      e.stopPropagation();
      return;
  ...
  if (inMenu) {
    ...
    if (sidebar.style.display === "none") {
      const sidebarContainer = document.querySelector(".sidebar-container-live");
      if (sidebarContainer) {
  ...
  if (isAdultChannel && parentalEnabled && !isChannelUnlocked && unlockSet) {
    ParentalPinDialog(
      () => {
        ...
        // Remove lock from only this specific card's logo wrapper
        const logoWrapper = focusedChannelItem.querySelector(".channel-card-logo-wrapper");
        if (logoWrapper) {
          logoWrapper.classList.remove("channel-card-locked");
        }
        const lockIcon = focusedChannelItem.querySelector(".channel-card-lock-icon");
        if (lockIcon) lockIcon.remove();
        ...
        if (selectedCategoryId !== "channelHistory") {
          ...
          const categoryList = qs(".livetv-channels-list");
          if (categoryList) {
            const categoriesToShow = filtered.slice(0, categoryChunk * pageSize);
...
LiveTvPage.cleanup = () => {
  ...
  document.removeEventListener('webkitfullscreenchange', handleFullscreenBorder);
  document.removeEventListener('mozfullscreenchange', handleFullscreenBorder);
  document.removeEventListener('MSFullscreenChange', handleFullscreenBorder);
