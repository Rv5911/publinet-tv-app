    ...
    const setFocus = (list, idx, cls) => {
        ...
        arr.forEach((el) => {
            ...
            const m = el.querySelector(
                ".livetv-channel-category-name, .channel-card-name"
            );
            if (m) m.classList.remove("marquee-active");
        });

        if (arr[idx]) {
            arr[idx].classList.add(cls);

            const m = arr[idx].querySelector(
                ".livetv-channel-category-name, .channel-card-name"
            );
            if (m) {
                const sw = m.scrollWidth;
                const cw = m.clientWidth;
                if (sw > cw) {
                    m.setAttribute("data-marquee", m.textContent);
                    m.style.setProperty("--scroll-dist", `-${sw - cw}px`);
                    m.style.setProperty("--duration", `${sw / 150}s`);
                    m.classList.add("marquee-active");
                }
            }
  ...
  `;
    ...
    setTimeout(() => {
        ...
        function keyHandler(e) {
            ...
            if (isBackKey(e)) {
                // If search input is focused and has content, delete last character
