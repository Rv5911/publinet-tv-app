...
const lastPlayedEpisodeId = localStorage.getItem("lastPlayedEpisodeId");
  ...
  if (seriesDetailData) {
    localStorage.setItem("seriesEpisodesData", JSON.stringify(seriesDetailData.episodes || {}));
  }
...
const rebuildFocusable = () => {
  ...
  const episodeItems = [...document.querySelectorAll(".seasons-episodes-item")];
...
const updatePlayButton = () => {
  ...
  console.log(continueWatchingEpisodes,"continueWatchingEpisodescontinueWatchingEpisodes")
  if (lastWatchedEpisode && lastWatchedEpisode.episodeId) {
    ...
    if (foundEpisode && foundSeason && foundSeasonKey) {
      ...
      playBtn.setAttribute("data-last-episode-index", 
      ...
      if (startOverBtn) {
        ...
        startOverBtn.setAttribute("data-last-episode-index", 
          episodes[foundSeasonKey].findIndex(ep => ep.id === foundEpisode.id));
  ...
  const firstSeasonKey = Object.keys(episodes).sort((a, b) => parseInt(a) - parseInt(b))[0];
...
const castHtml = castList
  .map(
    (castMember) => `<div class="series-cast-item" tabindex="0">
        ...
        }" alt="${castMember.original_name || "Unknown"}" class="series-cast-item-image"
...
const playEpisode = (episodeId, episodeIndex, startFromBeginning = false) => {
  ...
  if (episode) {
    ...
    if (startFromBeginning) {
      ...
      removeItemFromHistoryById(episodeId, "continueWatchingSeries", username);
      ...
      const updatedPlaylists = playlistsData.map(playlist => {
        if (playlist.playlistName === currentPlaylistName) {
          const updatedContinueWatching = (playlist.continueWatchingSeries || []).filter(
...
if (localStorage.getItem("currentPage") == "seriesDetailPage"&&localStorage.getItem("navigationFocus")=="seriesDetailPage") {
  ...
  if (isDropdownOpen) {
    ...
    if (e.key === "ArrowDown") {
      dropdownFocusIndex = Math.min(dropdownFocusIndex + 1, seasons.length - 1);
      updateDropdownFocus();
      return;
      ...
      if (!seasonObj) {
        console.warn("No season found for dropdownFocusIndex:", dropdownFocusIndex, seasons);
        hideDropdown();
        return;
      }
...
if (focused === playBtn) {
  if (playBtn.textContent.includes("Resume")) {
    ...
    const lastEpisodeIndex = parseInt(playBtn.getAttribute("data-last-episode-index"));
    ...
    const firstEpisodeIndex = parseInt(playBtn.getAttribute("data-last-episode-index"));
  ...
  const lastEpisodeIndex = parseInt(startOverBtn.getAttribute("data-last-episode-index"));
  const lastEpisodeId = startOverBtn.getAttribute("data-last-episode-id");
  ...
  if (lastSeason && lastEpisodeIndex !== null && lastEpisodeId) {
    ...
    const result = toggleFavoriteItem(selectedSeriesItem.series_id, "favouriteSeries");
    ...
    favBtn.setAttribute("aria-label", result.isFav ? "Remove from Favorites" : "Add to Favorites");
  ...
  const episodeIndex = parseInt(focused.getAttribute("data-episode-index"));
  ...
  if (continueWatchingMap[seasonKey] && continueWatchingMap[seasonKey][episodeKey] != null) {
  ...
  playEpisode(episodeId, episodeIndex, !hasProgress);
    ...
    const sidebar = document.querySelector(".sidebar-container-series-detail");
...
if (e.key === "ArrowRight") {
  ...
  if (focused === menuBtn) {
  ...
  else if (focused === playBtn) {
    if (startOverBtn) {
      currentFocusIndex = focusableEls.indexOf(startOverBtn);
    } else if (trailerBtn) {
      currentFocusIndex = focusableEls.indexOf(trailerBtn);
    } else {
    ...
    if (castBtn && focusableEls.includes(castBtn)) {
      ...
      // If no cast button, stay on seasons button or move to next available
      return;
    }
  } 
  else if (focused === castBtn) {
    ...
    const idx = episodeItems.indexOf(focused);
    if (idx < episodeItems.length - 1) {
      currentFocusIndex = focusableEls.indexOf(episodeItems[idx + 1]);
    }
  } 
  else if (castItems.includes(focused)) {
    const idx = castItems.indexOf(focused);
    if (idx < castItems.length - 1) {
      currentFocusIndex = focusableEls.indexOf(castItems[idx + 1]);
    }
  }
  setFocus(focusableEls[currentFocusIndex]);
}
...
if (e.key === "ArrowDown") {
  ...
  rebuildFocusable();
  const newEpisodeItems = [...document.querySelectorAll(".seasons-episodes-item")];
  const newCastItems = [...document.querySelectorAll(".series-cast-item")];
  ...
  if (focused === menuBtn) {
    ...
    if (newEpisodeItems.length > 0) {
      currentFocusIndex = focusableEls.indexOf(newEpisodeItems[0]);
    } else if (newCastItems.length > 0) {
      currentFocusIndex = focusableEls.indexOf(newCastItems[0]);
    }
  } 
  else if (focused === castBtn) {
    if (newEpisodeItems.length > 0) {
  ...
  // ✅ Back/Escape
  if (["Escape", "Back", "BrowserBack", "XF86Back"].includes(e.key) || e.keyCode === 10009) {
    e.preventDefault();
      ...
      <div class="series-detail-content">
        <div class="first-content">
          <div class="first-item">
            <p class="first">Directed By:</p>
            <p class="second">${seriesInfo.director || "N/A"}</p>
          </div>
          <div class="first-item">
            <p class="first">Release Date:</p>
            <p class="second">${seriesInfo.releaseDate || "N/A"}</p>
          </div>
          <div class="first-item">
            <p class="first">Genre:</p>
            <p class="second">${seriesInfo.genre || "N/A"}</p>
          </div>
          <div class="first-item">
            <p class="first">Plot:</p>
            <p class="second second-plot">${
              seriesInfo.plot || "No plot available."
            }</p>
          </div>
          <div class="first-item">
            <p class="first"></p>
<div class="series-detail-buttons">
    ...
    if (firstSeasonEpisodes.length > 0) {
      ...
      return `
        <button class="series-detail-play-button" tabindex="0"
          ...
          Play - S${firstSeasonKey.toString().padStart(2, '0')}.E${firstEpisode.episode_num.toString().padStart(2, '0')}
        </button>
      `;
    }
  ...
  <button class="series-detail-fav-button" tabindex="0">
    <span class="heart-icon">
      ${
        isItemFavoriteForPlaylist(selectedSeriesItem.series_id, "favouriteSeries")
          ? `<i class="fa-solid fa-heart"></i>`
          : `<i class="fa-regular fa-heart"></i>`
      }
    </span>
...
<img
  src="${seriesInfo.cover || "./assets/noImageFound.png"}"
...
/>            <div class="card-rating">
              ${Array.from(
                {
                  length: Math.min(
                    Math.floor(seriesInfo.rating_5based || 0),
                    5
                  ),
                },
                () =>
    ...
    <div class="series-detail-description">
      <p>${seriesInfo.plot || "No description available."}</p>
    </div>
