            ...
            switch (key) {
                case "ArrowDown":
                    ...
                    if (navState.focus === "carousel" || navState.focus === "watchNow") {
                        ...
                        const allCategoryLists = document.querySelectorAll(".home-card-list[data-category]");
                        const availableCategories = Array.from(allCategoryLists)
                            .map(list => parseInt(list.getAttribute("data-category")))
                            .filter(catId => {
                                const list = document.querySelector(`.home-card-list[data-category="${catId}"]`);
                        ...
                        const currentIndex = availableCategories.indexOf(navState.currentCategory);
                        if (currentIndex >= 0 && currentIndex < availableCategories.length - 1) {
