async function MovieDetailPage() {
    ...
    function initFocus() {
        ...
        // Check for return focus from trailer
        const returnSelector = localStorage.getItem("returnFocusSelector");
        if (returnSelector) {
            const returnEl = document.querySelector(returnSelector);
            if (returnEl && focusableEls.includes(returnEl)) {
                currentFocusIndex = focusableEls.indexOf(returnEl);
                setFocus(returnEl);
                localStorage.removeItem("returnFocusSelector");
                return;
            }
            localStorage.removeItem("returnFocusSelector");
        }
    ...
    function moviesDetailPageKeydownHandler(e) {
        if (
            ...
            if (e.key === "Enter") {
                ...
                if (focused === trailerBtn) {
                    if (movieDetailData.info && movieDetailData.info.youtube_trailer) {
                        ...
                        // Save focus for return
                        localStorage.setItem(
                            "returnFocusSelector",
                            ".movie-detail-more-info-button"
                        );
            ...
            // BUT: Only sync if we're not already navigating with arrow keys
            var isArrowKey = [
                "ArrowRight",
                "ArrowLeft",
                "ArrowUp",
            ...
            if (e.key === "ArrowRight") {
                if (currentFocusIndex < focusableEls.length - 1) {
                    currentFocusIndex++;
                }
            }
            if (e.key === "ArrowLeft") {
