...
function getAPISeriesCategories(sortType = "default") {
    ...
    const seriesByCategory = getSeriesByCategoryMap();
    ...
    for (let i = 0; i < allSeriesCategoriesData.length; i++) {
        ...
        let series = seriesByCategory.get(category.category_id) || [];
        ...
        series = deduplicateStreamsByName(series).slice(0, 50);
...
function loadSeriesChunk(category, categoryIndex) {
    ...
    let chunkSize =
        categoryIndex === 0 && !getSeriesSearchQuery() ?
        3 :
        seriesChunkLoadingState.horizontalChunkSize;
...
function createSeriesCategorySection(category, categoryIndex) {
    ...
    let viewAllRowClass =
        categoryIndex === 0 && !getSeriesSearchQuery() ? " view-all-row" : "";
    html +=
    ...
    if (currentCardIndex === "header") {
        ...
        if (card) {
            ...
            if (card.classList.contains("series-view-all-cats-btn")) {
                ...
                if (scrollContainer) {
                    const cardsScroll = scrollContainer.querySelector(
                        ".view-all-cards-scroll",
                    );
                    if (cardsScroll) {
...
function activateSeriesMarquee(card) {
    ...
    if (scrollWidth > clientWidth) {
        ...
        titleElement.style.setProperty("--duration", `${scrollWidth / 50}s`);
...
let seriesPageTimeout = null;

function SeriesPage() {
    if (seriesPageTimeout) clearTimeout(seriesPageTimeout);
    validateSeriesData();
    ...
    seriesPageTimeout = setTimeout(function() {
        const currentPlaylist = getCurrentPlaylist();
        ...
        recentlyAddedToTop = deduplicateStreamsByName(
        ...
        if (isSearching) {
            ...
            initialCategories = fixedTopCategories
                .concat(topCategories)
                .filter((category) => {
...
function getSeriesByCategoryMap() {
    ...
    if (seriesByCategoryCache && seriesByCategoryCache.data === streams) {
        return seriesByCategoryCache.map;
    }
    const map = new Map();
...
window.refreshSeriesSearchResults = function() {
    ...
    seriesSearchTimeout = setTimeout(() => {
        ...
        const favouriteSeries = [];
        const popularSeries = [];
        const recentSeriesArray = [];

        const allStreams = window.allSeriesStreams || [];
        for (let i = 0; i < allStreams.length; i++) {
            const s = allStreams[i];
            if (!s) continue;
            if (normalizeTextSeries(s.name).includes(q)) {
                ...
                if (resultsByCategoryId.get(cId).length < 50)
                    resultsByCategoryId.get(cId).push(s);

                if (favSet.has(sId) && favouriteSeries.length < 50)
                    favouriteSeries.push(s);
                if (s.rating_5based > 4 && popularSeries.length < 50)
                    popularSeries.push(s);
                if (recentSet.has(sId) && recentSeriesArray.length < 50)
                    recentSeriesArray.push(s);
            }
        }
        ...
        const allseriesCategoriesData = window.allseriesCategories || [];
        for (let i = 0; i < allseriesCategoriesData.length; i++) {
            const cat = allseriesCategoriesData[i];
            if (!cat) continue;
            const matches = resultsByCategoryId.get(String(cat.category_id));
            if (matches && matches.length > 0) {
                initialCategories.push({
                    title: cat.category_name || "Category",
                    series: matches,
                    id: cat.category_id,
                    containerClass: "series-category-container",
        ...
        container.innerHTML = createSeriesHeader();

        seriesChunkLoadingState.loadedChunks = {};
        seriesChunkLoadingState.loadedCategories = 0;
        ...
        function renderNextChunk() {
            ...
            if (cat.series.length > 7) {
                html += `<div class="series-view-more" data-category="${chunkIndex}" data-index="header" data-total="${cat.series.length}">
                          <span>See More (${cat.series.length})</span>
                          <i class="fas fa-chevron-right"></i>
                        </div>`;
            }
            html += "</div>";
            ...
            // Render cards
            const cardsToRender = Math.min(cat.series.length, 7);
            const size = cat.id === "popular" ? "large" : "normal";
            for (let j = 0; j < cardsToRender; j++) {
                const sData = formatSeriesData(cat.series[j]);
                if (sData) {
                    html += createSeriesCard(sData, size, chunkIndex, j);
                }
            }
